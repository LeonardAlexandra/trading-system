# Phase2.0 D3 证据包（E2E-2.0 约束触发可验证点）

## 修改文件清单
- 修改：`src/phase2/evaluator.py`
- 新增：`tests/e2e/test_e2e_phase2_constraint_violation.py`
- 新增：`docs/runlogs/phase20_d3_pytest_output.txt`
- 新增：`docs/runlogs/phase20_d3_pytest_output_with_markers.txt`
- 新增：`docs/Phase2.0_D3_证据包.md`

## E2E 测试代码
文件：`tests/e2e/test_e2e_phase2_constraint_violation.py`
```python
import json
...
## pytest 原始输出
### 指定命令
命令：`pytest tests/e2e/test_e2e_phase2_constraint_violation.py`
来源：`docs/runlogs/phase20_d3_pytest_output.txt`
```text
============================= test session starts ==============================
...
tests/e2e/test_e2e_phase2_constraint_violation.py .                      [100%]

============================== 1 passed in 0.08s ===============================
```

### 含审计输出
命令：`pytest tests/e2e/test_e2e_phase2_constraint_violation.py -s`
来源：`docs/runlogs/phase20_d3_pytest_output_with_markers.txt`
```text
============================= test session starts ==============================
platform darwin -- Python 3.11.7, pytest-9.0.2, pluggy-1.6.0
rootdir: /Users/zhangkuo/TradingView Indicator/trading_system
...
tests/e2e/test_e2e_phase2_constraint_violation.py D3_PHASE12_COUNTS_BEFORE={"decision_snapshot": 0, "execution_events": 0, "log": 0, "trade": 13}
D3_PHASE12_WRITE_SQL_COUNT=0
D3_REPORT_SAMPLE_JSON={"baseline_version_id": "D3-V-BASE", "comparison_summary": {"baseline": {"avg_holding_time_sec": null, "max_drawdown": 10.0, "realized_pnl": 80.0, "trade_count": 3, "win_rate": 0.666667}, "constraint_violations": ["max_risk_exposure"], "current": {"avg_holding_time_sec": null, "max_drawdown": 130.0, "realized_pnl": -80.0, "trade_count": 3, "win_rate": 0.6666666666666666}, "delta": {"max_drawdown": 120.0, "realized_pnl": -160.0, "trade_count": 0, "win_rate": -3.3e-07}}, "constraint_definition": {"custom": null, "max_drawdown_pct": 500.0, "max_risk_exposure": 10.0, "min_trade_count": 1}, "dd_fail_conclusion": "fail", "pass_conclusion": "pass", "risk_fail_conclusion": "fail", "trade_fail_conclusion": "fail"}
D3_PHASE12_COUNTS_AFTER={"decision_snapshot": 0, "execution_events": 0, "log": 0, "trade": 13}
.
...
## 报告 JSON 样本
```json
{"baseline_version_id": "D3-V-BASE", "comparison_summary": {"baseline": {"avg_holding_time_sec": null, "max_drawdown": 10.0, "realized_pnl": 80.0, "trade_count": 3, "win_rate": 0.666667}, "constraint_violations": ["max_risk_exposure"], "current": {"avg_holding_time_sec": null, "max_drawdown": 130.0, "realized_pnl": -80.0, "trade_count": 3, "win_rate": 0.6666666666666666}, "delta": {"max_drawdown": 120.0, "realized_pnl": -160.0, "trade_count": 0, "win_rate": -3.3e-07}}, "constraint_definition": {"custom": null, "max_drawdown_pct": 500.0, "max_risk_exposure": 10.0, "min_trade_count": 1}, "dd_fail_conclusion": "fail", "pass_conclusion": "pass", "risk_fail_conclusion": "fail", "trade_fail_conclusion": "fail"}
```
...
- AFTER
```json
{"decision_snapshot": 0, "execution_events": 0, "log": 0, "trade": 13}
```

## 与 AC 逐条对照说明
- [x] 约束触发时 conclusion 正确（pass 场景=pass；trade_count/max_drawdown/risk_exposure 场景=fail）。
- [x] comparison_summary 明确违规指标（constraint_violations 含 min_trade_count / max_drawdown_pct / max_risk_exposure）。
- [x] evaluation_report 持久化正确（查询返回且含完整 constraint_definition）。
- [x] baseline_version_id 未受影响（仍为 baseline strategy_version_id）。
- [x] 无写回/优化语义（conclusion 与 comparison_summary 均校验无“建议参数/写回/优化”）。
- [x] 无 Phase 1.2 写操作（`D3_PHASE12_WRITE_SQL_COUNT=0`，且前后行数一致）。
