...
| live_confirm_token 不匹配 | ORDER_REJECTED | LIVE_GATE_CONFIRM_TOKEN_MISSING | test_pr17a_confirm_token_mismatch_rejects |
...
### 场景 B：Live HTTP transient 5xx → 重试 → 断路器打开 → 拒绝 → 可回滚
- **复现**：Demo 路径，FakeOkx 返回 5xx；2 笔失败 → 第三笔 CIRCUIT_OPEN；调用 close_circuit + append_event(CIRCUIT_RESET_BY_OPERATOR)。
...
```
...
tests/integration/test_pr17b_live_risk_limits.py ...                     [ 51%]
...
============================= 145 passed in 2.67s ==============================
```

### 7.3）pytest -q tests/integration

```
...
tests/integration/test_pr17b_live_risk_limits.py::test_pr17b_notional_exceeded_rejects_no_http PASSED [ 78%]
tests/integration/test_pr17b_live_risk_limits.py::test_pr17b_hourly_limit_exceeded_rejects_no_http PASSED [ 79%]
tests/integration/test_pr17b_live_risk_limits.py::test_pr17b_daily_limit_exceeded_rejects_no_http PASSED [ 80%]
...
============================== 73 passed in 2.26s ==============================
```
...
| 修改 | tests/integration/test_pr17a_live_path_gates.py | test_pr17b_all_gates_pass_allows_create_order（门禁全过允许下单） |
| 修改 | tests/unit/execution/test_live_gate.py | live_enabled 参数；test_live_gate_pr17b_all_gates_pass_allowed；test_live_gate_live_enabled_false_rejects |
